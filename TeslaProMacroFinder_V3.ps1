...
$DetectedVendors = New-Object System.Collections.Generic.List[string]
...
function Line([string]$Char = "═", [ConsoleColor]$Color = [ConsoleColor]::DarkCyan) {
    $w = [Math]::Max(110, $Host.UI.RawUI.WindowSize.Width - 1)
...
function Header {
    ...
    Write-Host "    ██║   ███████╗███████║███████╗██║  ██║██║     ██║  ██║╚██████╔╝" -ForegroundColor Green
    Write-Host "    ╚═╝   ╚══════╝╚══════╝╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝  ╚═╝ ╚═════╝ " -ForegroundColor Green
    Write-Host "                             TeslaProMacroFinder V3" -ForegroundColor Cyan
    Write-Host "                      Precision Macro / Vendor Audit Console" -ForegroundColor DarkCyan
    Line
    Write-Host ""
}
...
function Spinner([string]$Text = "Loading", [int]$Loops = 12) {
    $frames = @('|','/','-','\')
...
function CutText([string]$Text, [int]$Max = 96) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "-" }
    if ($Text.Length -le $Max) { return $Text }
    return $Text.Substring(0, $Max - 3) + "..."
}

function Add-Hit([string]$Category,[string]$Vendor,[string]$Name,[string]$Path,[string]$Source,[string]$Reason) {
...
function KeywordHits([string]$Path) {
    $keywords = @(
        ...
        '#installkeybdhook','#installmousehook','settimer','~lbutton'
    )
...
function RelevanceScore($Name,$Path,$Keywords,$Vendor,$Category) {
    ...
    foreach ($k in @('autohotkey','ahk','lua','.ahk','.lua','#installmousehook','#installkeybdhook','settimer','~lbutton')) {
...
function Analyze-Item([object]$Item) {
    ...
    $score = RelevanceScore $Item.Name $Item.Path $keywords $Item.Vendor $Item.Category
...
function Export-Reports {
    ...
    $lines.Add("SUMMARY")
    $lines.Add("----------------------------------------------------------------")
    $lines.Add("Detected vendors  : $([string]::Join(', ', $DetectedVendors))")
    ...
    $lines.Add("3. Use a proper mouse button test site and press every button one by one.")
    $lines.Add("4. Repeat the exact same test with the vendor software fully CLOSED.")
    $lines.Add("5. If an extra physical button consistently registers as left click, treat that as a red flag.")
    $lines.Add("6. Use recent vendor config changes as supporting context, not sole proof.")
...
function Show-Dashboard {
    ...
    $run  = ($rows | Where-Object Running -eq 'YES').Count
    $cfg  = ($rows | Where-Object { $_.Category -match 'VendorConfig|UserConfig|Script|DeepScanHit' }).Count
    $sw   = ($rows | Where-Object { $_.Category -match 'InstalledApp|Process' }).Count

    Write-Host "┌──────────────────────────┬──────────────────────────┬──────────────────────────┐" -ForegroundColor DarkGray
    Write-Host ("│ Detected Vendors         │ Found Items             │ Running Vendor Proc.     │") -ForegroundColor White
    Write-Host ("│ {0,-24} │ {1,-24} │ {2,-24} │" -f ($(if($DetectedVendors.Count){[string]::Join(', ', $DetectedVendors)} else {'-'}), $rows.Count, $run)) -ForegroundColor Cyan
    Write-Host "├──────────────────────────┼──────────────────────────┼──────────────────────────┤" -ForegroundColor DarkGray
    Write-Host ("│ Vendor Software          │ Relevant Files          │ Deep Scan                │") -ForegroundColor White
    Write-Host ("│ {0,-24} │ {1,-24} │ {2,-24} │" -f $sw, $cfg, $(if($Deep){'ON'} else {'OFF'})) -ForegroundColor Cyan
    Write-Host "├──────────────────────────┼──────────────────────────┼──────────────────────────┤" -ForegroundColor DarkGray
    Write-Host ("│ HIGH Risk                │ MEDIUM Risk             │ LOW Risk                 │") -ForegroundColor White
    Write-Host ("│ {0,-24} │ {1,-24} │ {2,-24} │" -f $high, $med, $low) -ForegroundColor Cyan
    Write-Host "└──────────────────────────┴──────────────────────────┴──────────────────────────┘" -ForegroundColor DarkGray
    Write-Host ""

    Write-Host "VENDOR BREAKDOWN" -ForegroundColor Cyan
    Line "-" DarkGray
    ...
    Write-Host ""
    Write-Host "Kies een tab bovenaan voor detailpagina's." -ForegroundColor DarkGray
}
...
function Show-Cards([string]$ActiveTab,[string]$TitleText,[object[]]$Rows) {
    ...
    foreach ($x in $Rows) {
        ...
        Write-Host ("│ Name     : {0}" -f (CutText $x.Name 92)) -ForegroundColor White
        ...
        Write-Host ("│ Keywords : {0}" -f (CutText $x.Keywords 92)) -ForegroundColor Gray
        Write-Host ("│ Path     : {0}" -f (CutText $x.Path 92)) -ForegroundColor DarkGray
        Write-Host ("│ Reason   : {0}" -f (CutText $x.Reason 92)) -ForegroundColor DarkGray
        Write-Host "└────────────────────────────────────────────────────────────────────────────────────────────────┘" -ForegroundColor DarkGray
        Write-Host ""
    }
    Pause-Key
...
function Show-ManualChecks {
    ...
    Write-Host ""
    Write-Host "This page exists because on-board macros may only reveal themselves when software is fully closed." -ForegroundColor DarkGray
    Pause-Key
}
...
function Show-Exports {
    ...
    Write-Host "This scanner is strongest on software/config traces; on-board checks remain manual." -ForegroundColor Gray
    Pause-Key
}
...
function Run-Scan {
    ...
    $DetectedVendors.Clear()
    foreach ($v in (Detect-ActiveVendors)) { $DetectedVendors.Add($v) | Out-Null }
    ...
    $steps = @(
        ...
        @{P=78; T='Vendor configs en sterke user scripts scannen'; A={ Collect-VendorFiles $DetectedVendors; Collect-UserScripts }},
...
function Main-Menu {
    while ($true) {
        ...
        switch ($key) {
            'D1' { Show-Dashboard; Pause-Key }
            'NumPad1' { Show-Dashboard; Pause-Key }
            'D2' {
